...
parser.add_argument('--modality', default=None,
                    help='Filter specific modality if input is a folder (e.g. t1c, t2f, t2w, t1n). Default: all non-seg MRIs.')
...
if input_path.is_dir():
    ...
    files = []
    for name, image in gather_nifty(input_path):
        # Skip segmentation mask files
        if image.name.endswith('-seg.nii.gz') or image.name.endswith('_seg.nii.gz') or image.name.endswith('-seg.nii'):
            continue
        if args.modality and f"-{args.modality}.nii" not in image.name and f"_{args.modality}.nii" not in image.name:
            continue
        files.append([output_path / f'{name}.json', image])
else:
