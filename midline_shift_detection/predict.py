...
from dpipe.im.shape_ops import zoom, pad
...
def rescale_nii(predict):
    def wrapped(image):
        ...
        x = zoom(x, scale_factor, axis=[0, 1])

        contours, limits = predict(x)
        # rescale the predictions back
        contours = zoom(contours, 1 / scale_factor[0], axis=0) / scale_factor[1]
        limits = zoom(limits, 1 / scale_factor[0], axis=0)
